...
def client_login(request):
    ...
    if request.method == 'POST':
        ...
        try:
            user = ind_clientDT.objects.get(clint_email=email)
            if password == user.clint_password:
                request.session['clint_id'] = user.clint_id

                # Check if user needs to reset password or accept terms
                if not user.clint_password or not user.Term_condition:
                    return redirect('post_login_actions')
                
                return redirect('dashbord/')
            else:
                return redirect('/?msg=wrong password')
        except ind_clientDT.DoesNotExist:
            return redirect(f'/?msg=Email not registered')
...
def post_login_actions(request):
    if 'clint_id' not in request.session:
    ...
    subject = f'Dear {user.clint_name_first} {user.clint_name_last},'
    message = (
        f"Dear {user.clint_name_first} {user.clint_name_last},\n\n"
        "We would like to inform you that all subscription fees paid to 'Boomalgo' are non-refundable. "
        ...
        "does not indicate future performance of any current or future strategy, model, or advice provided by 'Boomalgo'.\n\n"
        ...
        "objectives of any strategy, model, or advice provided by 'Boomalgo' will be achieved.\n\n"
        
        "'Boomalgo', its partners, principal officers, and employees do not assure or guarantee any return on the investment "
        "in strategies, models, or advice given to the investor. The value of investments can fluctuate depending on various "
        "market factors and forces. 'Boomalgo' or its associates are not liable or responsible for any loss or shortfall arising "
    ...
    email_content = message  # Store email content to pass to the template
    
    if request.method == 'POST':
        if 'accept_terms' in request.POST:
            user.Term_condition = True
            user.save()

            # Send the email
            try:
                email_from = settings.EMAIL_HOST_USER
                recipient_list = [user.clint_email]
        ...
        if 'new_password' in request.POST:
            ...
            user.clint_password = new_password
            user.save()
            return JsonResponse({'success': True})

    context = {
        'reset_password': not user.clint_password,
        'accept_terms': not user.Term_condition,
        'email_content': email_content,  # Add email content to context
    }
    
    if not context['reset_password'] and not context['accept_terms']:
        return redirect('client_dashbord')
    
    return render(request, 'post_login_actions.html', context)
            ...
            try:
                ...
                try:
                    ...
                    **Important:** We strongly recommend that you change your login password to one of your choice at your earliest convenience.
                    ...
                    [http://ai.Boomalgo](http://ai.Boomalgo)
   ...
   # Get today's date
    ...
    if client_user.Term_condition == True:
            ...
            if broker_name == 'Angel One':
                ...
                redirect_uri = "https://google.boomalgo.com/handle-smartapi-redirect/"  # Your redirect URI
    ...
    if 'clint_id' in request.session:
        ...
        if str(client_user.broker) == 'Angel One':
            ...
            if auth_code:
                ...
                redirect_uri = "https://google.boomalgo.com/handle-smartapi-redirect/"
...
from .models import ind_clientDT, GROUP  # Import the necessary models

def signup_view(request):
    if request.method == 'POST':
        first_name = request.POST['clint_name_first']
        last_name = request.POST['clint_name_last']
        email = request.POST['clint_email']
        phone_number = request.POST['clint_phone_number']
        password = request.POST['clint_password']

        # Check if the email already exists
        if ind_clientDT.objects.filter(clint_email=email).exists():
            msg = 'Email already registered.'
            return render(request, 'signup.html', {'msg': msg, 'error': 'yes'})

        current_datetime = timezone.now()
        group, created = GROUP.objects.get_or_create(GROUP='Group1')

        try:
            new_client = ind_clientDT.objects.create(
                clint_name_first=first_name,
                clint_name_last=last_name,
                clint_email=email,
                clint_password=password,
                clint_phone_number=phone_number,
                clint_date_joined=current_datetime,
                clint_last_login=current_datetime + timedelta(days=2),
                client_Group=group,
                clint_plane='Trial',
                paid_paln=False,
                clint_status='Active',
...
def broker_detail(request, broker_name):
    # Aapke template ke path ko yahan specify karein
    template_name = f'brokers/{broker_name}.html'
